...
double measureHitRate(char* trace_file, int cache_size) {
    ...
    while (fscanf(file, "%d", &value) != EOF) {
        ...
        if (is_hit) {
            total_hits++;
            // Move the hit page to the front of the cache
            if (prev != NULL) {
            ...
            // If the cache is full, remove the least recently used page from the end
            ...
            // Insert the new page at the front of the cache
